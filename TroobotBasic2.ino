...
struct layer
{
  int cmd, // assertion command
    ...
    state; // layer state, used by AFSM tasks
}
...
const int numLayers = 2,
  /* The arbitration flags on the robot
   * are implemented as bit positions
   * in a 16 bit flag word 
   * (hence only 16 possible layers),
   * defined as follows:
   */
  SEARCH = 0x80,
  DEF = 0x100;

LAYER *layers[numLayers] = { &search, &halt };

/* The bit definitions of arbitration flags
 * apply to a set of global masks:
 */
int botFlags, // bit = layer asserting
  botAvoid, // bit = inverse response
  botEnable, // bit = enable this layer
  botSuppress; // bit = ignore this layer

volatile int targetActive,
  userEnable,
  userInvert,
  searchEnable,
  searchInvert;
...
int setRotVel, // [deg/s]
  ...
  targetRadius = 25, // error radius around target in cm
  downRamp = 100, // slow down within 100 cm of target
  minSpeedAtTarget = 5; // arbitrary speed units
...
void setup() 
{
  initBot(); // initialize system and tasks
...
void initPublishers()
{
  ...
  /* Create processes for group of concurrent tasks: 
   * Add processes to multitasking queue
   * Arguments: function, init param, stack size
   */
  //createProcess(sensors); // main sensor loop (primary subsumption loop controlling robot)
  //createProcess(odometers); // calculate robot position
...
int searchTask() // call this from x Hz (~20 Hz) sensor loop
{
  ...
  extern int targetActive; // global, reset when target acquired

  int distance, headingError; // values returned from virtual sensor

  if(searchEnable) search.flag = true;
  else search.flag = false;

  if(targetActive == 0) // do cruise behavior
  {
    if(searchInvert) search.cmd = 0; // decelerate
    else search.cmd = topVel; // or accelerate

    search.arg = 0; // straight ahead
  }
  else // do navigate behavior
  {
    locateTarget(target, &distance, &headingError); // read the "sensor"

    if(distance < targetRadius) // arrived at target?
    {
      search.cmd = 0; // yes, stop

      search.arg = 0; // stop turning

      targetActive = 0; // signal that target is acquired
    }
    else // still looking for target
    {
      if(distance < downRamp) // slow down when getting close
      {
        search.cmd = ( ( distance * topVel ) / downRamp );

        if(search.cmd < minSpeedAtTarget)
          search.cmd = minSpeedAtTarget;
      }
      else
        search.cmd = topVel; // distance > 100 cm, request top speed
...
void locateTarget(int target, int *distance, int *headingError)
{
  double x, y;
...
void arbitrate()
{
  ...
  if(botSystem & arbitrateMode)
  {
    j = ~botSuppress & botFlags; // suppress
    ...
    while((j & 1) == 0) // subsume
    {
      ...
      if(i == numLayers - 1) break; // default
    }

    motorCmd(layers[i]); // execute, given pointer to winning layer
  }
